...
from ops import *
from utils import *
from tensorflow.contrib.data import prefetch_to_device, shuffle_and_repeat, map_and_batch, batch_and_drop_remainder
...
class CGAN(object):
    ...
    def __init__(self, sess, args):
        ...
        self.gen_all_dir = args.gen_all_dir

        self.y_dim = args.y_dim  # dim of the autoencoder edoder o/p
        ...
        if self.dataset_name == 'mnist':
            ...
            self.conv_autoe_features = load_conv_autoe_features(y_dim=self.y_dim, dataset_name=self.dataset_name)
    ...
    def generator(self, z, y, is_training=True, reuse=False):
        with tf.variable_scope("generator", reuse=reuse):
            # merge noise and latent as label
            z = concat([z, y], 3)
            ch = 1024
    ...
    def discriminator(self, x, y, is_training=True, reuse=False):
        with tf.variable_scope("discriminator", reuse=reuse):
            # merge image and latent as label
    ...
    def build_model(self):
        ...
        if self.custom_dataset:
            Image_Data_Class = ImageData(self.img_size, self.c_dim, self.augment_flag)
            inputs = tf.data.Dataset.from_tensor_slices(self.data)
            inputs = inputs.map(Image_Data_Class.image_processing, num_parallel_calls=16)

            y = tf.data.Dataset.from_tensor_slices(self.conv_autoe_features)

            # zip the two datasets together
            dcomb = tf.data.Dataset.zip((inputs, y))

            gpu_device = '/gpu:3'
            dcomb = dcomb.apply(shuffle_and_repeat(self.dataset_num)) \
                .apply(batch_and_drop_remainder(self.batch_size)) \
                .apply(prefetch_to_device(gpu_device, self.batch_size))

            i_iterator = dcomb.make_one_shot_iterator()

            self.inputs, self.y = i_iterator.get_next()
            ...
            self.inputs = tf.placeholder(tf.float32, [self.batch_size, self.img_size, self.img_size, self.c_dim],
                                         name='real_images')
            # latent
            self.y = tf.placeholder(tf.float32, [self.batch_size, 1, 1, self.y_dim], name='y')
        ...
        real_logits = self.discriminator(self.inputs, self.y)

        # output of D for fake images
        fake_images = self.generator(self.z, self.y)
        fake_images_recon = self.generator(self.z, self.y, reuse=True)
        fake_logits = self.discriminator(fake_images, self.y, reuse=True)
        ...
        recon_loss = L1_loss(x=fake_images_recon, y=self.inputs)  # reconstruction

        # get loss for discriminator
        self.d_loss = discriminator_loss(self.gan_type, real=real_logits, fake=fake_logits) + GP

        # get loss for generator
        self.g_loss = self.adv_w*generator_loss(self.gan_type, fake=fake_logits)+\
    ...
    def gen(self):
        ...
        self.saver = tf.train.Saver()
        could_load, checkpoint_counter = self.load(self.checkpoint_dir)
        gen_dir = os.path.join(self.gen_dir, self.model_dir)
        check_folder(gen_dir)
        ...
        n_batches = math.ceil(self.dataset_num / self.batch_size)


        """ random condition, random noise """

        for j in range(38): # generate samples 4 times the size of the dataset len
            for i in range(n_batches) : # generate the samples of size datset len
                z_sample = np.random.uniform(-1, 1, size=(self.batch_size, 1, 1, self.z_dim))

                samples = self.sess.run(self.fake_images, feed_dict={self.z: z_sample})

                save_images_gen(samples, gen_dir + '/' + self.model_name + '_outer_{}_gen_batch_{}'.format(j,i))
    ...
    def gen_all(self):
        ...
        self.saver = tf.train.Saver()
        could_load, checkpoint_counter = self.load(self.checkpoint_dir)
        gen_dir = os.path.join(self.gen_all_dir, self.model_dir)
        check_folder(gen_dir)
        ...
        for i in range(n_batches) : # generate the samples of size datset len
            ...
            save_images_gen(samples, gen_dir + '/' + self.model_name + '_gen_batch_{}'.format(i))
